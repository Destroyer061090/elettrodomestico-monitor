...
def _uid(iid, sfx, slot): return f"{DOMAIN}_{iid}_{sfx}_x{slot}"
def _eid(sfx, slot):      return f"{sfx}_x{slot}"
...
class _TempoPeriod(_Base):
    ...
    @property
    def extra_state_attributes(self): return {"periodo_precedente": self._d.get(self._lk, "0min")}
...
class _CostoPeriod(_Base):
    ...
    def extra_state_attributes(self):
        return {
            "periodo_precedente": self._d.get(self._lk, 0.0),
            "fonte_costo":        self._d.get("cost_source", "fisso"),
            "costo_per_unita":    self._d.get("cost_per_unit", 0.0),
        }
...
class _Schedule(_Base):
    ...
    def native_value(self) -> str:
        on = self._d.get("schedule_auto_on", "00:00:00")
        try:
            h, m = int(on.split(":")[0]), int(on.split(":")[1])
        except (IndexError, ValueError):
            h = m = 0
        return "Disabilitato" if (h == 0 and m == 0) else on[:5]
    ...
    def extra_state_attributes(self):
        ...
        try:
            ...
            off_disp = "Disabilitato" if (h == 0 and m == 0) else off[:5]
        except (IndexError, ValueError):
            off_disp = "Disabilitato"
        attrs = {
            "accensione":      d.get("schedule_auto_on",  "")[:5],
...
class _Master(_Base):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        return {
            ...
            ATTR_VERSION:           d.get("version",            "4.6"),
...
class _Versione(_Base):
    ...
    @property
    def native_value(self): return self._d.get("version", "4.6")
