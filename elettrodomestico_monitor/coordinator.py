...
class ElettrodomesticoCoordinator(DataUpdateCoordinator):
    ...
    def _push(self) -> None:
        """Publish a fresh build to entities only when data actually changed.
        Must be called from within the event loop (async context). For calls
        from @callback / sync contexts, use _push_safe() instead."""
        data = self._build()
        if data == self._last_pushed:
            return
        self._last_pushed = data
    ...
    def _push_safe(self) -> None:
        """Schedule _push on the next event-loop iteration.

        async_set_updated_data (inside _push) triggers async_write_ha_state on
        all subscriber entities. HA raises a RuntimeError if that happens while
        we're already inside a state-change callback. Deferring with call_soon
        exits the current callback frame first, so the write is always safe.
        """
        self.hass.loop.call_soon(self._push)
    ...
    def _ac_off(self) -> None:
        ...
        # Use _push_safe (schedules on the event loop) — calling _push()
        # directly from a @callback context raised the thread-safety error.
        self._push_safe()
